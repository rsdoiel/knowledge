...
func TestEligibleConcepts_RequiresMoreThanOneOccurrence(t *testing.T) {
	...
	got, err := kb.EligibleTagConcepts("chunking mentioned once here", []string{"chunking"})
	if err != nil {
		t.Fatalf("EligibleTagConcepts: %v", err)
	}
	if len(got) != 0 {
...
func TestEligibleConcepts_IncludesTermMentionedTwice(t *testing.T) {
	...
	got, err := kb.EligibleTagConcepts("chunking here, chunking again", []string{"chunking"})
	if err != nil {
		t.Fatalf("EligibleTagConcepts: %v", err)
	}
...
func TestEligibleConcepts_ExcludesCandidatesNotInAllowedList(t *testing.T) {
	...
	got, err := kb.EligibleTagConcepts("chunking chunking workspace workspace", []string{"chunking"})
	if err != nil {
		t.Fatalf("EligibleTagConcepts: %v", err)
	}
...
func TestEligibleConcepts_ExcludesCodeSpans(t *testing.T) {
	...
	got, err := kb.EligibleTagConcepts("saw `const format = x` once, then `unsupported format` again", []string{"format"})
	if err != nil {
		t.Fatalf("EligibleTagConcepts: %v", err)
	}
	if len(got) != 0 {
...
// ─── determinism (found 2026-09-23 comparing two builds, DR-0037) ───────────
//
// EligibleTagConcepts ranged over a map and returned names in arbitrary order,
// so `document frontmatter` printed its known-concept proposals in a different
// order on every run.

func TestEligibleTagConcepts_ResultIsSortedAndStableAcrossCalls(t *testing.T) {
	kb := openTestKB(t)
	names := []string{"zebra", "Apple", "mango", "banana", "Cherry", "kiwi", "olive"}
	var text string
	for _, n := range names {
	...
	want := []string{"Apple", "banana", "Cherry", "kiwi", "mango", "olive", "zebra"}
	for i := 0; i < 25; i++ {
		got, err := kb.EligibleTagConcepts(text, names)
		if err != nil {
			t.Fatalf("EligibleTagConcepts: %v", err)
		}
		if strings.Join(got, ",") != strings.Join(want, ",") {
			t.Fatalf("call %d: got %v, want %v (case-insensitive alphabetical, every time)", i, got, want)
		}
	}
}

func TestInsertWikilink_WrapsNamesThatBeginOrEndInPunctuation(t *testing.T) {
	cases := []struct{ name, text, want string }{
		{"C++", "we use C++ here", "we use [[C++]] here"},
		{"F#", "written in F#.", "written in [[F#]]."},
		{".NET", "a .NET app", "a [[.NET]] app"},
	}
	for _, c := range cases {
		got, ok := insertWikilink(c.text, c.name)
		if !ok || got != c.want {
			t.Errorf("insertWikilink(%q, %q) = %q, %v; want %q, true", c.text, c.name, got, ok, c.want)
		}
	}
}

func TestInsertWikilink_SkipsAPunctuationNameGluedToAWordAndUsesALaterMention(t *testing.T) {
	got, ok := insertWikilink("ABC++ first, then C++ properly", "C++")
	if !ok || got != "ABC++ first, then [[C++]] properly" {
		t.Errorf("got %q, %v; want the standalone mention wrapped, not the glued one", got, ok)
	}
}
